...
def download_models(folder: str):
    ...
    for file_name, url in download_files.items():
        ...
        if not os.path.exists(output_path):
            print(f"Downloading file: {file_name} to {output_path}")
...
def load_hamer(data_dir: str):
    from pathlib import Path
    from ..configs import get_config

    checkpoint_path = os.path.join(data_dir, 'hamer_ckpts', 'checkpoints', 'hamer.ckpt')
    ...
    model_cfg.MANO.DATA_DIR = os.path.join(data_dir, 'data/')
    model_cfg.MANO.MEAN_PARAMS = os.path.join(
        data_dir, 'data/mano_mean_params.npz')
    model_cfg.MANO.MODEL_PATH = os.path.join(data_dir, 'data/mano')
    model_cfg.EXTRA.FOCAL_LENGTH = 500.0
    # model_cfg.MANO.MEAN_PARAMS = os.path.join(data_dir, 'hamer_ckpts/checkpoints/hamer.ckpt')
